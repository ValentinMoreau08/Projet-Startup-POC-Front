<template>
  <f7-page>
    ...
    <div
      class="row no-gap"
      v-if="user.manager !== undefined && user.manager !== null"
    >
      <div class="col"><strong> Manager: </strong></div>
      <div class="col" @click="getUser(user.manager.id)">
    ...
    <f7-list v-if="user.role !== undefined && user.role.id == 2">
      ...
      <f7-list-item
        v-for="managedUser in user.managed"
        :key="managedUser.id"
        @click="getUser(managedUser.id)"
      >
      ...
      <f7-list-item accordion-item title="Les temps en détail">
        <f7-accordion-content>
          <f7-block>
            <f7-list
              media-list
              v-if="user.times !== undefined && user.times !== null"
            >
              <f7-list-item
...
export default {
  async created() {
    ...
    await Axios.get("http://localhost:8180" + "/managers")
      .then(response => {
    ...
    await Axios.get("http://localhost:8180" + "/users")
      .then(response => {
